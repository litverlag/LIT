class LektorController < TableController
  def index
    ...
    respond_to do |format|
      ...
      format.json {serve_table_data(Gprod.joins(:buch, :lektor, :titelei, :preps, :umschlag, :druck, :binderei, :final).where("lektoren.id": current_user.lektor.id), displayedColumns, filterableColumns, editable, notViewable, "Gprods.id")}
    end
  end
  ...
  def produktion
    ...
    respond_to do |format|
      ...
      format.json {serve_table_data(Gprod.joins(:buch, :lektor).where("lektoren.id": current_user.lektor.id), displayedColumns, filterableColumns, editable, notViewable, "Gprods.id")}
    end
  end
